...
map2Any <- function(features, unique_id=c("PROBEID", "ENSEMBL", "ENTREZID", "SYMBOL"), map_to, unique_id_col, platform=NULL, tables_path="/home/rebecca/omicon/mapping_tables/2022-11-02", keep_all=F){
  ...
  colnames(features)[colnames(features)=="SYMBOL"] <- "SYMBOL.y" 
  ...
  if(sum(duplicated(mapped_ids$UNIQUE.ID))>0){
    ...
    if(keep_all){ ## ...keep all identifiers, and collapse rows with one-to-many mappings:

      mapped_ids <- suppressMessages({mapped_ids %>%
...
mapAlias2Symbol <- function(features, unique_id_col, tables_path="/home/rebecca/omicon/mapping_tables/2022-11-02", keep_all=F, fill_NAs=F){
  ...
  mapping_table <- fread(list.files(
    pattern="ALIAS", 
    path=file.path(tables_path, "homo_sapiens_mapping_tables/SYMBOL"), 
    full.names=T
  ), data.table=F)
  ...
  return(data.frame(UNIQUE.ID=features1, SYMBOL=mapped_ids$SYMBOL, features[,-c(unique_id_col)]))
